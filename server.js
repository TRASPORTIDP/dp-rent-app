...
[uploadDir, contractsDir, firmeDir].forEach(dir => {
...
button,.btn{background:#b30000;color:white;border:0;padding:10px;border-radius:6px;text-decoration:none;display:inline-block;font-weight:bold;cursor:pointer;}
.btn2{background:#333;}
...
function generaPdfContratto(id, callback) {
  ...
  `, [id], (err, p) => {
    ...
    doc.fontSize(18).text(AZIENDA.nome, { align: 'center' });
    doc.fontSize(10).text(AZIENDA.indirizzo, { align: 'center' });
    doc.text(`P.IVA / CF ${AZIENDA.piva} - Tel. ${AZIENDA.telefono}`, { align: 'center' });
    doc.text(`Email: ${AZIENDA.email}`, { align: 'center' });

    doc.moveDown();
    doc.fontSize(16).text('CONTRATTO DI NOLEGGIO', { align: 'center' });

    doc.moveDown();
    doc.fontSize(12).text(`Contratto n.: ${p.codice}`);
    doc.text(`Stato: ${p.stato || 'bozza'}`);
    doc.text(`Data creazione: ${p.created_at || ''}`);

    doc.moveDown();
    doc.fontSize(13).text('ANAGRAFICA CLIENTE', { underline: true });
    doc.fontSize(10);
    doc.text(`Cliente: ${p.nome || ''} ${p.cognome || ''}`);
    doc.text(`Telefono: ${p.telefono || ''}`);
    doc.text(`Email: ${p.email || ''}`);
    doc.text(`Codice fiscale: ${p.codice_fiscale || ''}`);
    doc.text(`Indirizzo: ${p.indirizzo || ''}`);
    doc.text(`Comune/CAP: ${p.citta || ''} ${p.cap || ''}`);
    doc.text(`Tipo cliente: ${p.tipo_cliente || ''}`);
    doc.text(`P.IVA: ${p.piva || ''}`);
    doc.text(`Ragione sociale: ${p.ragione_sociale || ''}`);

    doc.moveDown();
    doc.fontSize(13).text('DETTAGLI DELLA PRENOTAZIONE', { underline: true });
    doc.fontSize(10);
    doc.text(`Check-out: ${p.data_inizio} ore ${p.ora_inizio || '08:30'} - Narni`);
    doc.text(`Check-in: ${p.data_fine} ore ${p.ora_fine || '18:00'} - Narni`);
    doc.text(`Giorni tariffati: ${p.giorni}`);
    doc.text(`Ritiro serale: ${p.ritiro_serale ? 'SI (+30 euro + IVA)' : 'NO'}`);

    doc.moveDown();
    doc.fontSize(13).text('VEICOLO', { underline: true });
    doc.fontSize(10);
    doc.text(`Targa: ${p.targa || ''}`);
    doc.text(`Marca/Modello: ${p.marca || ''} ${p.modello || ''}`);
    doc.text(`Categoria: ${p.categoria || ''}`);
    doc.text(`Km inclusi totali: ${Number(p.km_inclusi || 0) * Number(p.giorni || 0)}`);
    ...
    doc.moveDown();
    doc.fontSize(13).text('RIEPILOGO NOLEGGIO', { underline: true });
    doc.fontSize(10);
    ...
    doc.text(`Deposito cauzionale: euro ${Number(p.cauzione || CAUZIONE).toFixed(2)}`);

    doc.moveDown();
    doc.fontSize(13).text('CONDIZIONI GENERALI E PRIVACY', { underline: true });
    doc.fontSize(9);
    doc.text('Il cliente dichiara di aver preso visione e accettare le condizioni generali di noleggio e l’informativa privacy DP RENT / Trasporti DP S.R.L.');
    doc.text(`Condizioni generali: ${TERMS_URL}`);
    doc.text(`Informativa privacy: ${PRIVACY_URL}`);

    doc.moveDown();
    doc.fontSize(9);
    doc.text('Condizioni principali: veicolo consegnato con il pieno e da riconsegnare con il pieno; extra km euro 0,15/km ove previsto; danni, multe, pedaggi, franchigie, smarrimenti e costi accessori a carico del cliente.');

    doc.moveDown(2);
    if (p.firma_path && fs.existsSync(p.firma_path)) {
      doc.fontSize(11).text('Firma cliente acquisita digitalmente:');
      doc.image(p.firma_path, { fit: [220, 80] });
    } else {
      doc.fontSize(11).text('Firma cliente: ______________________________');
    }
    ...
    db.all(`SELECT * FROM prenotazioni WHERE stato!='annullato'`, [], (e2, pren) => {
      ...
      mezzi.forEach(m => {
        ...
        for (let d = 1; d <= days; d++) {
          ...
          rows += occ
            ? `<td class="occupato" title="${occ.codice} ${occ.nome || ''}">O</td>`
            : `<td class="libero">L</td>`;
        }
