...
/* V223 SUPER: grafica pulita, mobile-first */
...
<body>
<header>${logoHtml}<h1>DP RENT APP <small style="font-size:13px;color:#ddd">V223 SUPER STABILE</small></h1></header>
<nav class="dp-super-nav">
<a href="/">Dashboard</a>
...
async function queryDisponibilita(mezzo_id, data_inizio, data_fine, ora_inizio='08:30', ora_fine='18:00', excludeId=0) {
  ...
  return get(`
    ...
    WHERE mezzo_id = ?
    AND COALESCE(LOWER(stato),'') NOT IN ('annullato','eliminato','eliminato_attesa','cancellato','scaduto','bozza','preventivo','preventivo_whatsapp','attesa_si_no','richiesta_cliente','tariffa_riservata','non_confermato','rientrato','chiuso','completato')
    AND id <> COALESCE(?,0)
...
app.get('/', async (req, res) => {
  try {
    const mezzi = await get(`SELECT COUNT(*) as tot FROM mezzi`).catch(()=>({tot:0}));
    const pren = await get(`SELECT COUNT(*) as tot FROM prenotazioni`).catch(()=>({tot:0}));
    const atteseRowsV137 = await v137AtteseRows().catch(()=>[]);
    const allMezzi = await all(`SELECT * FROM mezzi`).catch(()=>[]);
    const alertsCount = (allMezzi||[]).map(m => alertMezzo(m)).filter(Boolean).length;
    res.send(page('Dashboard', `
      <div class="dp-one-page">
        <section class="dp-home-hero"><h2>DP RENT</h2><p>Gestionale veloce — pronto per iPhone, iPad e Mac</p></section>
        <section class="dp-home-alerts">
          <a class="dp-alert-card" href="/richieste-attesa" style="text-decoration:none;color:#111"><span>🔴</span> <b>${atteseRowsV137.length}</b> Clienti in attesa</a>
          <a class="dp-alert-card warn" href="/scadenze-mezzi" style="text-decoration:none;color:#111"><span>🟡</span> <b>${alertsCount}</b> Scadenze / alert</a>
        </section>
        <section class="dp-home-grid">
          <a class="dp-home-card primary" href="/nuova-prenotazione"><span class="ico">＋</span>Prenotazione<small>Crea richiesta o contratto</small></a>
          <a class="dp-home-card" href="/planning"><span class="ico">📅</span>Planning<small>Giorno / settimana / mese</small></a>
          <a class="dp-home-card" href="/mezzi-web"><span class="ico">🚗</span>Mezzi<small>Parco veicoli</small></a>
        ...
        <p style="text-align:center;font-weight:900;color:#666;margin:22px 0">Mezzi: ${mezzi?.tot || 0} • Contratti: ${pren?.tot || 0} • V223 SUPER STABILE</p>
      </div>
    `));
  } catch(e) {
    res.status(500).send(page('Errore dashboard', `<div class="box"><h2 class="bad">Errore dashboard</h2><pre>${esc(e.message)}</pre></div>`));
  }
});


app.get('/avanzate', async (req,res)=>{ res.send(page('Avanzate', `<div class="dp-one-page"><section class="dp-home-hero"><h2>Avanzate</h2><p>Funzioni amministrative DP RENT</p></section><section class="dp-home-grid"><a class="dp-home-card" href="/clienti"><span class="ico">👥</span>Clienti</a><a class="dp-home-card" href="/scansione-documenti"><span class="ico">📷</span>Scansione documenti</a><a class="dp-home-card" href="/documenti-clienti"><span class="ico">📂</span>Documenti clienti</a><a class="dp-home-card" href="/import-mezzi"><span class="ico">📊</span>Import Excel</a><a class="dp-home-card" href="/cargos"><span class="ico">🚚</span>Ca.R.G.O.S.</a><a class="dp-home-card" href="/cargos-config"><span class="ico">⚙️</span>Config CARGOS</a><a class="dp-home-card" href="/richieste-attesa"><span class="ico">🚨</span>Clienti in attesa</a><a class="dp-home-card" href="/logo"><span class="ico">🎨</span>Logo</a></section></div>`)); });
app.get('/storico', (req,res)=>res.redirect('/prenotazioni'));
app.get('/video-mezzi', async (req,res)=>{ const mezzi=await all(`SELECT * FROM mezzi ORDER BY targa, marca, modello`).catch(()=>[]); const cards=(mezzi||[]).map(m=>`<div class="dp-video-card"><h2>🎥 ${esc(m.targa||'')} - ${esc(m.modello||m.marca||'')}</h2><div class="dp-video-actions"><a class="btn" href="/video-mezzi/${m.id}">Gestisci video</a></div></div>`).join('') || '<div class="box">Nessun mezzo trovato.</div>'; res.send(page('Video mezzi', `<div class="dp-one-page"><section class="dp-home-hero"><h2>Video mezzi</h2><p>Un video attuale per ogni targa</p></section>${cards}</div>`)); });
app.get('/video-mezzi/:id', async (req,res)=>{ const m=await get(`SELECT * FROM mezzi WHERE id=?`,[req.params.id]).catch(()=>null); if(!m)return res.status(404).send(page('Mezzo non trovato',`<div class="box"><h2 class="bad">Mezzo non trovato</h2></div>`)); let folder=null,videos=[],err=''; try{folder=await dpV223FindVideoFolderByTarga(m.targa); if(folder)videos=await dpV223ListVideoFiles(folder.id);}catch(e){err=e.message;} const current=videos[0]||null; res.send(page('Video mezzo', `<div class="dp-one-page"><div class="dp-video-card"><h2>🎥 ${esc(m.targa)} - ${esc(m.modello||m.marca||'')}</h2>${folder?`<p class="ok"><b>Cartella collegata:</b> ${esc(folder.name)}</p>`:`<p class="bad"><b>Cartella non trovata.</b> Controlla DP_RENT_VIDEO_FOLDER_ID e cartella che inizia con ${esc(m.targa)}.</p>`}${err?`<pre>${esc(err)}</pre>`:''}<div class="dp-video-actions"><a class="btn btn2" href="/video-mezzi">Torna video mezzi</a>${folder?`<a class="btn" target="_blank" href="${esc(folder.webViewLink||'')}">📂 Apri cartella Drive</a>`:''}</div>${current?`<div class="dp-video-current"><p><b>Video attuale:</b><br>${esc(current.name)}</p><a class="btn" target="_blank" href="${esc(current.webViewLink||'')}">▶️ Apri video su Drive</a>${videos.length>1?`<p class="warn">Trovati ${videos.length} video: al prossimo caricamento l'app prova a eliminare i vecchi.</p>`:''}</div>`:`<p class="notice">Nessun video attuale letto nella cartella.</p>`}<form class="dp-upload-box" method="POST" action="/video-mezzi/${m.id}/upload" enctype="multipart/form-data"><h3>⬆️ Carica / sostituisci video</h3><p>Prima elimina i vecchi video della cartella, poi carica quello nuovo.</p><input type="file" name="video" accept="video/*,.mov,.mp4" required><button>Carica e sostituisci</button></form><form method="POST" action="/video-mezzi/${m.id}/delete" onsubmit="return confirm('Eliminare video e duplicati?')"><button class="bad" type="submit">🗑️ Elimina video attuale</button></form></div></div>`)); });
app.post('/video-mezzi/:id/upload', upload.single('video'), async (req,res)=>{ try{ const m=await get(`SELECT * FROM mezzi WHERE id=?`,[req.params.id]); if(!m||!req.file)throw new Error('Mezzo o file mancante'); const folder=await dpV223FindVideoFolderByTarga(m.targa); if(!folder)throw new Error('Cartella video non trovata per targa '+m.targa); const del=await dpV223DeleteVideoFiles(folder.id).catch(e=>({deleted:0,errors:[e.message]})); const name=dpV223VideoFileName(m,req.file.originalname); const up=await dpV223UploadVideoToFolder(req.file.path,name,req.file.mimetype,folder); res.send(page('Video caricato',`<div class="box"><h2 class="ok">Video caricato</h2><p><b>Mezzo:</b> ${esc(m.targa)} ${esc(m.modello||'')}</p><p><b>Eliminati prima:</b> ${del.deleted||0}</p>${del.errors&&del.errors.length?`<pre>${esc(del.errors.join('\n'))}</pre>`:''}<p><a class="btn" target="_blank" href="${esc(up.webViewLink||folder.webViewLink||'')}">Apri video/cartella Drive</a></p><a class="btn btn2" href="/video-mezzi/${m.id}">Torna al mezzo</a></div>`)); }catch(e){res.status(500).send(page('Errore upload video',`<div class="box"><h2 class="bad">Errore upload video</h2><pre>${esc(e.message)}</pre><a class="btn" href="/video-mezzi/${req.params.id}">Torna</a></div>`));} });
app.post('/video-mezzi/:id/delete', async (req,res)=>{ try{ const m=await get(`SELECT * FROM mezzi WHERE id=?`,[req.params.id]); if(!m)throw new Error('Mezzo non trovato'); const folder=await dpV223FindVideoFolderByTarga(m.targa); if(!folder)throw new Error('Cartella video non trovata'); const del=await dpV223DeleteVideoFiles(folder.id); res.send(page('Video eliminato',`<div class="box"><h2 class="ok">Video eliminati: ${del.deleted}</h2>${del.errors&&del.errors.length?`<pre>${esc(del.errors.join('\n'))}</pre>`:''}<a class="btn" href="/video-mezzi/${m.id}">Torna al mezzo</a></div>`)); }catch(e){res.status(500).send(page('Errore elimina video',`<div class="box"><h2 class="bad">Errore elimina video</h2><pre>${esc(e.message)}</pre><a class="btn" href="/video-mezzi/${req.params.id}">Torna</a></div>`));} });
...
// =========================
// V223 SUPER STABILE: PDF reali, video pulito, stati preventivo
// =========================
...
async function dpV223ListVideoFiles(folderId){
  ...
  const r=await d.files.list({q:`'${folderId}' in parents and trashed=false and (mimeType contains 'video/' or name contains 'VIDEO_')`,fields:'files(id,name,mimeType,webViewLink,webContentLink,createdTime,modifiedTime)',orderBy:'modifiedTime desc',spaces:'drive',supportsAllDrives:true,includeItemsFromAllDrives:true});
  return r.data.files||[];
}
async function dpV223DeleteVideoFiles(folderId){
  const d=await dpV223DriveApi(); const files=await dpV223ListVideoFiles(folderId); let deleted=0, errors=[];
  for(const f of files){ try{ await d.files.delete({fileId:f.id,supportsAllDrives:true}); deleted++; }catch(e){ errors.push(`${f.name}: ${e.message}`); } }
  return {deleted,errors};
}
async function dpV223UploadVideoToFolder(localPath,fileName,mimeType,folder){
  if(!process.env.DRIVE_WEBAPP_URL) throw new Error('DRIVE_WEBAPP_URL mancante: serve Apps Script per caricare video nel Drive personale');
  await assertFileReadyV173(localPath,'Video');
  const base64=fs.readFileSync(localPath).toString('base64');
  const r=await fetch(process.env.DRIVE_WEBAPP_URL,{method:'POST',headers:{'Content-Type':'text/plain;charset=utf-8'},body:JSON.stringify({folderId:process.env.DP_RENT_VIDEO_FOLDER_ID||process.env.VIDEO_DRIVE_FOLDER_ID,subfolder:folder.name,filename:fileName,mimeType:mimeType||'video/quicktime',base64})});
...
app.get('/prenotazione/:id', async (req, res) => {
  ...
  res.send(page('Dettaglio contratto', `
    <div class="box">
      ...
      <p><b>Date:</b> ${esc(dpV223DateIt(p.data_inizio))} ore ${esc(p.ora_inizio)} - ${esc(dpV223DateIt(p.data_fine))} ore ${esc(p.ora_fine)}</p>
...
app.get('/planning', async (req, res) => {
  ...
  const pren = await all(`SELECT p.*, m.targa, m.marca, m.modello, m.categoria AS mezzo_categoria
    ...
    WHERE COALESCE(LOWER(p.stato),'') NOT IN ('annullato','eliminato','eliminato_attesa','cancellato','scaduto','bozza','preventivo','preventivo_whatsapp','attesa_si_no','richiesta_cliente','tariffa_riservata','non_confermato')
...
EXTRA KM RIENTRO: percorsi ${c.kmPercorsi}, concordati ${c.inclusi}, extra ${c.extraKm}, supplemento €${v180Money(c.supplemento)} IVA inclusa.` : `
KM RIENTRO: percorsi ${c.kmPercorsi}, concordati ${c.inclusi}, nessun extra km.`;
...
app.get('/cargos', async (req, res) => {
  ...
  const trs = rows.map(p => {
    ...
    return `<tr>
      ...
      <td>${esc(p.targa)}</td>
      <td>${esc(dpV223DateIt(p.data_inizio))} - ${esc(dpV223DateIt(p.data_fine))}</td>
...
app.get('/admin/fix-stati-preventivi-v223', async (req,res)=>{ try{ const r=await run(`UPDATE prenotazioni SET stato='attesa_si_no', tipo_record='preventivo_whatsapp' WHERE (tipo_record='preventivo_whatsapp' OR stato IN ('preventivo_whatsapp','richiesta_cliente')) AND COALESCE(LOWER(stato),'') NOT IN ('confermato','contratto','firmato','in_corso','checkout','check-out','rientrato','chiuso','annullato','non_confermato')`); res.send(page('Fix stati V223',`<div class="box"><h2 class="ok">Fix stati preventivi completato</h2><p>Righe aggiornate: ${esc(r?.changes||0)}</p><p>I preventivi WhatsApp non bloccano più il planning.</p><a class="btn" href="/planning">Apri planning</a><a class="btn btn2" href="/">Dashboard</a></div>`)); }catch(e){res.status(500).send(page('Errore fix stati',`<div class="box"><h2 class="bad">Errore</h2><pre>${esc(e.message)}</pre></div>`));} });
...
app.get('/contratto/:id/gestisci', async (req,res)=>{
  // V223: aprire/gestire non trasforma preventivo in contratto.
  const p=await get(`SELECT * FROM prenotazioni WHERE id=?`,[req.params.id]);
...
generaPdfContratto = async function v108GeneraPdfContrattoLocked(id, opts = {}) {
  ...
  const job = Promise.resolve().then(async () => {
    ...
    // V223: generare/vedere PDF non cambia stato al preventivo.
